...
# Create image zeroed
dd if="/dev/zero" of="${IMAGE_FILE}" bs=1M count=3072 conv=sync 2>/dev/null
...
sudo umount "${BINARIES_DIR}/p4" 2>/dev/null || true
...
sudo umount "${BINARIES_DIR}/p4"
rmdir "${BINARIES_DIR}/p1"
rmdir "${BINARIES_DIR}/p3"
rmdir "${BINARIES_DIR}/p4"
